...
environment.once("ready", () => {
    ...
    environment.discribe("App Testing", () => {
        ...
        environment.assert(environment.get<AppConfig>("App").SMS_PORT === Number(process.env.SMS_PORT), `environment.App.SMS_PORT should return ${Number(process.env.SMS_PORT)}`);
    })
    environment.discribe("Auth Testing", () => {
        environment.log(`environment.Auth.AUTH_FILE`, `${environment.get("Auth").AUTH_FILE}`);
        environment.log(`testing environment.Auth.AUTH_FILE!= null It should return false`);
        environment.assert(environment.get("Auth").AUTH_FILE == null, "environment.Auth.AUTH_FILE should be different from null");
    });
});
